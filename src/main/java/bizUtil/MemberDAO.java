...
public class MemberDAO {
		...
		try {
			...
			if (RS.next()) {
				...
				do {
					...
					memberVO.setUploadfile(RS.getString("uploadfile"));
	...
	public int mInert(MemberVO mvo) {
		...
		try {
			...
			PST.setString(8, mvo.getUploadfile());
	...
	public int mUpdate(MemberVO mvo) {
		...
		String sql = "update member set password = ?, name = ?, lev = ?, birthd = ?, point = ?, weight = ?, uploadfile = ? where id = ?";
		try {
			CN = JDBCUtil.getConnection();
			PST = CN.prepareStatement(sql);
			...
			PST.setString(7, mvo.getUploadfile());
			PST.setString(8, mvo.getId());
